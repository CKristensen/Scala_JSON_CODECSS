...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction { case Json.Num(x) => if (x.isValidInt) {
      Some(x.toInt)
    } else {
      None
    }
  ...
  implicit val stringDecoder: Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(x) => x.toString }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] = {
    Decoder.field[String]("name").
      zip(Decoder.field[Int]("age")).transform[Person](a => Person(a._1,a._2.toInt))
    }
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.fromFunction((c: Contacts) => Json.Obj(
      Map("people" -> Json.Arr(
        c.people map (x => Json.Obj(
          Map("name" -> Json.Str(x.name), "age" -> Json.Num(x.age))
        ))))))
  ...
  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.fromFunction {
      case Json.Obj(maip) =>
        maip("people") match {
          case Json.Arr(lst) => Option(Contacts(
            lst map (x  => implicitly[Decoder[Person]].decode(x) match {
              case Some(e) => e
            })))
            }
        }
